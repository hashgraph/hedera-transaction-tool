...
async function handleExecuted(
  ...
  if (success && receipt) {
    ...
    toast.success(`Node Update Transaction Executed`, { position: 'bottom-right' });
  }
}
...
const createTransaction = async () => {
  // TODO: gossip endpoints and service endpoints required
  ...
  if (nodeData.newNodeAccountId) {
    transaction.setAccountId(nodeData.newNodeAccountId);
  }
...
<template>
  <div class="flex-column-100 overflow-hidden">
    <form @submit="handleCreate" class="flex-column-100">
      <TransactionHeaderControls heading-text="Node Update Transaction">
        <template #buttons>
          <div
      ...
      <div class="fill-remaining">
        ...
        <div class="row align-items-end mt-6">
          <div class="form-group" :class="[columnClass]">
            <label class="form-label">New Node Account ID</label>
            <AppInput
              :model-value="nodeData.newNodeAccountId?.toString()"
              @update:model-value="v => (nodeData.newNodeAccountId = formatAccountId(v))"
              :filled="true"
              placeholder="Enter New Node Account ID"
            />
          </div>
        </div>
        ...
        <div class="row align-items-end mt-6">
          <div class="form-group" :class="[columnClass]">
            <label class="form-label">Gossip CA Certificate</label>
            <AppInput
