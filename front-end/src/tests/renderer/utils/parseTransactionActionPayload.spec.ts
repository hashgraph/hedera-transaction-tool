...
describe('parseTransactionActionPayload', () => {
  ...
  it('uses most-severe-wins: status_update takes priority over update', () => {
    const result = parseTransactionActionPayload([
  ...
  it('preserves status_update when update comes after it in the batch', () => {
    const result = parseTransactionActionPayload([
      { transactionIds: [1], eventType: 'status_update' },
      { transactionIds: [2], eventType: 'update' },
    ]);

    expect(result!.eventType).toBe('status_update');
  });

  it('uses update when no status_update is present', () => {
    const result = parseTransactionActionPayload([
      { transactionIds: [1], eventType: 'update' },
      { transactionIds: [2], eventType: 'update' },
    ]);
