    ...
    async clickOnFinalNextButtonWithRetry(retryCount = 5) {
        ...
        while (attempts < retryCount && !isSuccessful) {
            try {
                ...
                await this.window.waitForSelector(this.settingsButtonSelector, {state: 'visible', timeout: 1000});
                isSuccessful = true; // If the above waitForSelector doesn't throw, we assume success
            } catch (error) {
                console.log(`Attempt ${attempts + 1} to click ${this.finalNextButtonSelector} failed, retrying...`);
