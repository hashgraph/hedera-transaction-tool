...
export abstract class BaseNatsConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    this.consumePromise = this.consumeWithReconnect().catch(err =>
      this.logger.error('Unexpected consumer failure', err instanceof Error ? err.stack : String(err)),
    );
  }
  ...
  async onModuleDestroy() {
    ...
    // Stop the active message iterator so `for await` unblocks immediately
    if (this.messages) this.messages.stop();
