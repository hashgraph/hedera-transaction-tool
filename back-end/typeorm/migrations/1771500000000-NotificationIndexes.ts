...
export class NotificationIndexes1771500000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(
            `CREATE INDEX IF NOT EXISTS "IDX_notification_type_entityId"
             ON "notification" ("type", "entityId")`
        );
        await queryRunner.query(
            `CREATE INDEX IF NOT EXISTS "IDX_notification_receiver_userId_isRead"
             ON "notification_receiver" ("userId", "isRead")`
        );
        await queryRunner.query(
            `CREATE INDEX IF NOT EXISTS "IDX_notification_receiver_notificationId"
