...
- The `transaction_approver_list_member` row stores a `signature` over the transaction bytes and the public key used to produce it. These are verifiable — but any defense that relies solely on data within the database fails against an attacker with direct database write access. They can modify the signature, the key reference, or any other field to construct an apparently valid record.
- External records (e.g., HCS) do not solve this either. The client must know *which* HCS topic or external resource to trust for verification — and if that identifier is stored in the database, an attacker with DB access can change it to point to a topic they control, where they have written fake events. The attack simply moves one level up.
- This is the classic trust anchor problem: any reference stored in the database can be changed by someone who owns the database. The cycle can only be broken by: (a) hardcoding the trust anchor in the client binary at build time so it cannot be changed without a software release, or (b) placing the anchor under a separate credential domain that does not share the application database's access controls.
- The practical conclusion: **database access is equivalent to full system compromise**. The correct defense is operational security at that boundary — not a technical mechanism within the application itself.
- **Suggested security model (defense in depth):**
  - *Separate read from write.* Most people who need to query the database only need SELECT. Grant humans a `readonly` role; only the application user gets INSERT/UPDATE on sensitive tables (`transaction_approver_list`, `transaction_approver_list_member`).
  - *Immutability triggers.* Use database-level `BEFORE UPDATE` triggers to block changes to columns that should never change once set. Two cases:
    - On `user_key`: once `public_key_bytes` is set, it cannot be updated. A key is never "changed" — it is soft-deleted and replaced with a new row. This prevents the attack of swapping a user's public key to one the attacker controls.
    - On `transaction_approver_list_member`: once `approved` and `actioned_at` are set, those columns cannot be changed — an approval or disapproval cannot be silently overwritten.
    - A PostgreSQL superuser can disable a trigger (`ALTER TABLE ... DISABLE TRIGGER`), make the change, then re-enable it. This bypasses the block but is itself a DDL statement that appears in the `pgaudit` log — making it a detectable anomaly during external log review.
  - *External audit log.* Enable `pgaudit` (or equivalent) and ship all DML logs to an external aggregator (CloudWatch, Splunk, etc.) that database administrators cannot modify. Tampering may not be preventable, but it becomes detectable and attributable.
  - *Separate credential management.* Store the application's database credentials in a secrets manager (Vault, AWS Secrets Manager) with access controls independent of the database itself. The people who can query the database should not be able to read the application's credentials, and vice versa — compromising both requires breaching two separate systems.
  - *Residual risk.* Someone with both superuser database access and access to the credential store can still impersonate the application. There is no technical solution at that level. The defense is operational: minimize who holds both, require multi-party approval for superuser credential use, and treat any superuser session as a security event subject to review.
